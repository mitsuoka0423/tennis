# Watch 画面カタログ

**状態: 現行**（2026-07-27 作成）

Watch アプリの画面・状態・操作の一覧。**改善要望を画面単位でやり取りするための呼び名**を定める。
指定は「画面名/状態名 の 要素名」の形で行う（例: `計測/計測中 の停止ボタン`）。

関連: [F-W1〜W9 の要求](REQUIREMENTS.md#52-watch-側)

> **スクリーンショットは載せない。** UI を触るたびに古くなり、PNG では差分が読めないため。
> 見た目は SwiftUI プレビューで確認する（各 View に `#Preview` がある）。

---

## 1. 画面

遷移する画面は3つ。うち計測が3状態、モニターが2状態を持つ。

| 画面 | 役割 | ソース |
|---|---|---|
| **モード選択** | 起動直後。計測とモニターのどちらを使うか選ぶ | `RootView.swift` |
| **計測** | スイングを記録する（F-W1〜W6） | `ContentView.swift` |
| **モニター** | 加速度・角速度を波形で見る。保存しない（F-W9） | `SensorMonitorView.swift` |

計測とモニターは**同時に使えない**。モニターは `CMMotionManager`、計測は `CMBatchedSensorManager`
を使い、並走すると 200Hz の計測レートへ影響しうるため（詳細は 4章）。

---

## 2. 状態遷移図

ノードは画面と状態、有向エッジは操作。

```mermaid
stateDiagram-v2
    [*] --> モード選択 : アプリ起動

    モード選択 --> 認可ダイアログ : 初回起動時に自動表示
    認可ダイアログ --> モード選択 : 「許可」または「許可しない」

    モード選択 --> 待機 : 「計測」をタップ
    モード選択 --> 取得中 : 「モニター」をタップ

    state 計測 {
        待機 --> 開始処理中 : 「計測開始」をタップ
        開始処理中 --> 計測中 : ワークアウト確立に成功
        開始処理中 --> 計測エラー表示 : 開始に失敗（認可拒否など）
        計測中 --> 待機 : 「停止・保存」をタップ
        計測中 --> 計測エラー表示 : 収集中・停止時のエラー
        計測エラー表示 --> 待機 : 「OK」（発生元が開始処理中・停止時）
        計測エラー表示 --> 計測中 : 「OK」（発生元が計測中）
    }

    state モニター {
        取得中 --> 停止中 : 「一時停止」をタップ
        停止中 --> 取得中 : 「再開」をタップ（波形は積み直し）
        取得中 --> 取得中 : スケールをタップ（±4/8/16g・±500/1000/2000）
        停止中 --> 停止中 : スケールをタップ
        取得中 --> モニターエラー表示 : センサー取得に失敗
        モニターエラー表示 --> 停止中 : 「OK」
    }

    待機 --> モード選択 : 戻る
    取得中 --> モード選択 : 戻る（取得を停止）
    停止中 --> モード選択 : 戻る
```

図に現れない事実:

* **開始処理中・計測中からモード選択へ戻る辺は無い。** ナビゲーションバーごと隠しているため、
  停止・保存を経由するしかない。
* エラー表示はアラートであり、下に元の状態が残る。そのため戻り先が発生元によって分かれる。
* スクロールは状態も表示内容も変えないため辺に含めない。スケールのタップは状態を変えないが、
  ユーザー操作として必ず使うため自己遷移として含める。
* 認可ダイアログは iOS のシステム表示（初回起動時のみ）。

---

## 3. 画面ごとの構成要素

### 3.1 モード選択

状態を持たない。

| 要素 | 内容 |
|---|---|
| タイトル | 「Tennis Analyser」 |
| 計測行 | 「スイングを記録する」→ 計測画面へ |
| モニター行 | 「加速度・角速度を波形で見る」→ モニター画面へ |

### 3.2 計測

#### 待機（`!isRecording && !isStarting`）

| 要素 | 内容 |
|---|---|
| ナビゲーションバー | 「計測」。左端の戻るでモード選択へ |
| アイコン | テニスの人物（緑） |
| 説明文 | 「スイングを記録します」 |
| 計測開始ボタン | 緑。押すと開始処理中へ |

#### 開始処理中（`isStarting`）

ヘルスケアの認可待ちとワークアウトセッション確立中。通常は一瞬で計測中へ移る。

| 要素 | 内容 |
|---|---|
| インジケーター | 円形のプログレス（緑） |
| 見出し | 「準備中...」 |
| 補足 | 「ヘルスケアの許可が求められる場合があります」 |

#### 計測中（`isRecording`）

| 要素 | 内容 |
|---|---|
| 録画ドット | 赤い丸。計測中であることの表示 |
| 経過時間 | `MM:SS`。等幅の大きな数字 |
| スイング | 検知数と未転送数「n (残m)」 |
| レート | 実測Hzとロス率。1%未満=緑 / 5%未満=黄 / それ以上=赤 |
| 停止・保存ボタン | 赤。押すと待機へ戻る |

### 3.3 モニター

縦にスクロールする。加速度と角速度で同じ構成が2組並ぶ。

#### 取得中（`isRunning`）

| 要素 | 内容 |
|---|---|
| 見出し | 「加速度 g」「角速度 °/s」 |
| スケール | 右上の「±8g」「±1000」。タップで候補を循環 |
| 波形 | 3軸を重ねた1枚。X=赤 / Y=緑 / Z=シアン。中央が0、右端が最新、横軸3秒 |
| 現在値 | 波形の下に X / Y / Z の数値 |
| 状態行 | 緑のドットと実測Hz（画面下端） |
| 一時停止ボタン | オレンジ（画面下端） |

#### 停止中（`!isRunning`）

| 要素 | 内容 |
|---|---|
| 波形 | 停止した時点の3秒間が残る。流れない |
| 状態行 | ドットが灰色。Hz は最後の値のまま |
| 再開ボタン | 緑。押すと取得中へ戻り、波形は空から積み直す |

---

## 4. 部品

画面ではないため単独では現れない。

| 部品 | 役割 | ソース |
|---|---|---|
| **波形** | 3軸を1枚に重ねて描く。モニター画面が加速度・角速度で2枚使う | `SensorMonitorView.swift` / `AxisTraceChart` |

縦軸は固定スケール。候補は加速度 4 / 8 / 16 g、角速度 500 / 1000 / 2000 °/s
（既定は 8g と 1000°/s）。範囲を超えた値は上下でクリップする。
調整箇所は `SensorChartScale`。

---

## 5. 保留中: 呼び名と UI 文言の不一致

本書の呼び名は会話用に決めたもので、**画面上の文言はまだ追随していない**。

| 箇所 | 現在の文言 | 提案 |
|---|---|---|
| モード選択の項目名 | 「センサー」 | 「モニター」 |
| モニター画面のタイトル | 「センサー」 | 「モニター」 |
| モニターの状態行 | 「計測中」 | 「取得中」（計測画面と紛らわしいため） |

反映するか、呼び名は会話用と割り切って画面上は現状のままにするかは未決定。
