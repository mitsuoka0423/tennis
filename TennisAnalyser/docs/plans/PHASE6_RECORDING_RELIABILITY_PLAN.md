...
| W3 | 中間データ削除の安全化 | W2 |
| W4 | 診断記録のUI表示と共有 | W2 |
| W5 | 検出精度（閾値・re-arm・ラベル） | T11 のみ独立。T13→T12 は W2 に依存 |
...
- [ ] **T13: 実データへのラベル付与（T11 で発見。W2 完了後に着手）**
  - 保全データ328件すべてで `ShotClass` が空欄であり、精度を測る土台が無い
  - **付与手段は実装済み**。`ContentView.applyBulkTag`（一括）と
    `SwingDetailView`（個別）が既にある。不足しているのはラベルそのもの
  - ラベルが空欄なのは**録画不具合の二次被害**。波形だけでは球種を判別できず、
    判別にはクリップが要るが 325件中1件しか生成されなかった
  - よって W1/W2 で録画が継続するようになり、次回の実機検証でクリップが揃ってから着手する
