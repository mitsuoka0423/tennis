...
- [ ] **P3-T2: iOS — アノテーション UI（F-I3）**
  ...
  - 一覧（`ContentView`）: 各行にラベルチップを表示（未設定は視覚的に区別）
  - 一覧: 複数選択モード（Edit）で選択したスイングに一括タグ付け
  - DoD: ビルド green

- [ ] **P3-T3: iOS — 一覧のフィルタリング（F-I2 将来項目の実装）**
  - ツールバーにフィルタメニュー（すべて/未タグ/クラス別）
  - DoD: ビルド green

- [ ] **P3-T4: iOS — 解析エンジンの基礎（F-I4）**
  - 純粋関数として実装（テスト容易性、3.2節の方針に従う）: ピーク時刻（インパクトからの相対秒）、
    ピーク後の減速開始点（ピークの50%まで低下した最初の時刻）
  - `SwingDetailView` に指標を表示（「簡易指標」と明記し、スコアリング未実装であることを示す）
  - DoD: ビルド green、ユニットテスト追加
